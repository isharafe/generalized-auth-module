...
Admin controllers delegate to focused catalog, user, operations, data-transfer, and URL-inventory
services; controllers never access repositories directly. Configuration codes are immutable.
Permission codes are type-qualified, so their resource type is immutable as well. DELETE
soft-disables coded configuration, while
