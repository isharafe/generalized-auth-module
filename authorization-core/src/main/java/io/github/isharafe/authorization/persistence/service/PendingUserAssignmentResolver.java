...
public class PendingUserAssignmentResolver {
  ...
  public int resolve(AuthenticatedIdentity identity) {
    ...
    int resolved = 0;
    for (PendingUserAssignmentEntity assignment :
        pending.findByIssuerAndSubject(identity.issuer(), identity.subject())) {
      if (assignment.getTargetType() == AssignmentTargetType.ROLE) {
  ...
  private void invalidateAfterCommit(AuthenticatedIdentity identity) {
    afterCommit.execute(() -> cache.invalidateIdentity(identity));
  }
}
