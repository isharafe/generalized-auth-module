...
public class AuthorizationAdminService {
  ...
  public AdminDtos.EffectiveEntitlements effectivePermissions(Long userId) {
    ...
    value.permissions().stream()
        .sorted(Comparator.comparing(com.example.authorization.domain.Permission::code))
        .map(
            permission ->
  ...
  public AdminDtos.Page<AdminDtos.AuditEvent> audit(
      ...
      Pageable pageable) {
    return page(
        auditEvents.search(nullable(eventType), nullable(actor), nullable(target), pageable),
        this::auditEvent);
  }
  ...
  private void changedAll(String type, String target, String action) {
    afterCommit(cache::invalidateAll, adminEvent(type, target, action));
  }

  private void changedRules(String type, String target, String action) {
    afterCommit(cache::invalidateResourceRules, adminEvent(type, target, action));
  }
  ...
  private void changedIdentity(
    ...
    AuthenticatedIdentity identity =
        new AuthenticatedIdentity(user.getIssuer(), user.getSubject(), user.getUsername());
    afterCommit(() -> cache.invalidateIdentity(identity), adminEvent(type, target, action));
  }

  private AdminAuditEvent adminEvent(String type, String target, String action) {
  ...
  private void afterCommit(Runnable invalidation, AdminAuditEvent event) {
    Runnable callback =
        () -> {
          invalidation.run();
          audit.publish(event);
  ...
  private void publishNow(String type, String target, String action) {
    audit.publish(adminEvent(type, target, action));
  }
}
